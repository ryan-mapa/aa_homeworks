...
class MetaCorgiSnacks
  ...
  #   tasty = "get_#{method_name}_tastiness"
  #   if ["treat", "kibble", "bone"].include?(method_name)
  #     result = "#{method_name.capitalize}: " + @snack_box.send(info, @box_id) + ": #{@snack_box.send(tasty, @box_id)}"
  #     @snack_box.send(tasty, @box_id) > 30 ? "* #{result}" : result
  #   else
  #     super
  #   end
  # end
  ...
  def self.define_snack(name)
    define_method(name) do
      ...
      tasty = @snack_box.send("get_#{method_name}_tastiness", @box_id)
